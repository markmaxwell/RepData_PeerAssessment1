...
```{r,echo=TRUE}
# Calculate the mean of steps taken per day and create new aggregated dataframe
mean.steps.per.day <- ddply(activity, .(date), function(df) mean(df$steps,na.rm=TRUE))
# Rename second column name
colnames(mean.steps.per.day)[2] <- "steps"
# Return the 5-minute interval with the max number of steps across all the averages
